...
async function loadDropdowns() {
  ...
  if (classes) {
    ...
    classes.forEach(c => {
      ...
      classSelect.appendChild(new Option(`${sem}${c.name}`, c.id))
      batchClassSelect.appendChild(new Option(`${sem}${c.name}`, c.id))
    })
  }
}
...
function filterData() {
  ...
  const filtered = allPayments.filter(p => {
    ...
    const tMatch = p.title.toLowerCase().includes(keyword)
    const cMatch = p.classes && p.classes.name.toLowerCase().includes(keyword)
...
paymentForm.addEventListener('submit', async (e) => {
  ...
  } catch (err) { alert('儲存失敗：' + err.message) } finally { btn.disabled = false; btn.textContent = '儲存收費單' }
})

window.deletePayment = async (id, title) => {
  if (!confirm(`確定要刪除「${title}」這筆收費單嗎？`)) return
...
batchForm.addEventListener('submit', async (e) => {
  ...
  try {
    const targetClassId = batchClassSelect.value
    ...
    if (!roster || roster.length === 0) { alert('該班級目前沒有學生，無法開單。'); btn.disabled = false; btn.textContent = '確認批次開單'; return }

    // 2. 準備陣列批次寫入
    const payloads = roster.map(r => ({
      student_id: r.student_id, class_id: targetClassId, title: title, amount: amount, status: '未繳'
    }))

    const { error } = await supabase.from('payments').insert(payloads)
    if (error) throw error

    alert(`✅ 成功！已為該班級的 ${payloads.length} 位學生開立「${title}」待繳費單。`)
    window.closeBatchModal(); fetchPayments()
  } catch (err) { alert('批次開單失敗：' + err.message) } finally { btn.disabled = false; btn.textContent = '確認批次開單' }
})
  ...
  unpaid.forEach(p => {
    ...
    unpaidList.innerHTML += `
      <tr>
        ...
        <td>
          <button class="btn btn-primary" onclick="window.quickPay('${p.id}')" style="padding:4px 10px; font-size:13px; background:#10b981; border:none;">收款結清</button>
        </td>
      </tr>`
  })
...
window.quickPay = async (paymentId) => {
  ...
  if (!confirm('確定以「現金」方式於今日結清此筆款項嗎？')) return
  ...
  return `
    ...
    <head>
      ...
      <style>
        ...
        .cut-line { text-align: center; color: #94a3b8; margin-bottom: 20px; font-size: 12px; letter-spacing: 2px; }
        ...
        .total-box { margin-top: 20px; text-align: right; font-size: 20px; font-weight: bold; background: #eff6ff; padding: 15px; border-radius: 8px; color: #1e3a8a; }
    ...
    <body>
      <div class="cut-line">✂ -------------- 請沿虛線剪下 -------------- ✂</div>
      <div class="bill-container">
        <h1 class="header-title">補習班 繳費憑單</h1>
...
window.printSingleSlip = (paymentId) => {
  ...
  printWin.document.write(generatePrintHTML(`${p.students?.name}_${p.title}`, content))
...
window.printConsolidatedBill = () => {
  ...
  if (unpaid.length === 0) { alert('此學生目前沒有待繳費項目。'); return }
  ...
  `
  const printWin = window.open('', '_blank')
  printWin.document.write(generatePrintHTML(`${student?.name}_合併帳單`, content))
  printWin.document.close()
}
