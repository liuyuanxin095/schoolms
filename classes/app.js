...
const rosterStudentSelect = document.getElementById('roster-student-select')
...
async function initData() {
  ...
  const { data: tData } = await supabase.from('staff').select('id, name').in('role', ['teacher', 'manager', 'superadmin'])
  if (tData) tData.forEach(t => teacherSelect.appendChild(new Option(t.name, t.id)))
...
async function fetchClasses() {
  let query = supabase.from('classes').select('*, branches(name), staff!classes_teacher_id_fkey(name), classrooms(name)')
...
function renderTable(data) {
  ...
  data.forEach(c => {
    ...
    const classroomName = c.classrooms ? c.classrooms.name : '-'
    ...
    row.innerHTML = `
      ...
      <td>${c.semester || '-'}</td>
      <td>${branchName}</td>
      <td><span class="material-symbols-outlined" style="font-size:16px; vertical-align:middle; color:var(--text-light);">person</span> ${teacherName}</td>
      <td><div style="font-size:13px;">${c.schedule || '-'}</div><div style="font-size:12px; color:var(--primary); margin-top:2px;">${classroomName}</div></td>
      <td>
...
function filterData() {
  ...
  const filtered = allClasses.filter(c => {
    const matchKey = c.name.toLowerCase().includes(keyword) || (c.staff && c.staff.name.toLowerCase().includes(keyword))
...
window.openFormModal = (id = null) => {
  ...
  if (id) {
    ...
    if (c) {
      ...
      document.getElementById('classroom_id').value = c.classroom_id || ''; document.getElementById('schedule').value = c.schedule || ''
...
classForm.addEventListener('submit', async (e) => {
  ...
  try {
    ...
    const payload = {
      ...
      classroom_id: document.getElementById('classroom_id').value || null, schedule: document.getElementById('schedule').value || null,
      start_date: document.getElementById('start_date').value || null, end_date: document.getElementById('end_date').value || null
    }
...
window.openRosterModal = async (classId, className) => {
  ...
  const { data: students } = await supabase.from('students').select('id, name, student_number').order('name')
...
async function loadRosterList() {
  ...
  data.forEach(r => {
    ...
    rosterList.innerHTML += `
      <tr>
        <td><strong>${r.students.name}</strong></td>
        <td>${r.students.student_number || '-'}</td>
        <td><button class="btn-icon" style="color:var(--danger);" onclick="window.removeStudentFromClass('${r.student_id}')"><span class="material-symbols-outlined" style="font-size:18px;">person_remove</span></button></td>
      </tr>`
  })
}

window.addStudentToClass = async () => {
  const sId = rosterStudentSelect.value; if (!sId) return alert('請先選擇學生！')
  try {
    const { error } = await supabase.from('class_students').insert([{ class_id: currentManageClassId, student_id: sId }])
    if (error) { if(error.code === '23505') alert('此學生已在班級中！'); else throw error; return; }
    await loadRosterList()
  } catch (err) { alert('加入失敗：' + err.message) }
}

window.removeStudentFromClass = async (studentId) => {
  if (!confirm('確定將此學生移出班級嗎？')) return
  await supabase.from('class_students').delete().match({ class_id: currentManageClassId, student_id: studentId })
  await loadRosterList()
}
