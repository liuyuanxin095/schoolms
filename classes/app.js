...
let allClasses = []; let allClassrooms = []; let currentManageClassId = null; let scheduleSlots = []; let allStudents = [];
...
async function initData() {
  ...
  const { data: cData } = await supabase.from('classrooms').select('id, name, branch_id')
  if (cData) allClassrooms = cData
...
function renderTable(data) {
  ...
  data.forEach(c => {
    ...
    if (c.schedule) {
      ...
      slots.sort((a, b) => (dayOrder[a.substring(0, 2)]||99) - (dayOrder[b.substring(0, 2)]||99))
      scheduleHtml = `<div style="display:flex; gap:5px; flex-wrap:wrap;">` + slots.map(s => `<span style="background:#eff6ff; color:#2563eb; border:1px solid #bfdbfe; padding:4px 8px; border-radius:4px; font-size:12px; font-weight:600; white-space:nowrap;">${s}</span>`).join('') + `</div>`
    }
    const row = document.createElement('tr')
    row.innerHTML = `
      <td><strong>${c.name}</strong></td><td>${c.semester || '-'}</td><td>${c.branches ? c.branches.name : '-'}</td>
      <td><div style="margin-bottom:4px;"><span class="material-symbols-outlined" style="font-size:14px; vertical-align:text-bottom; color:var(--primary);">history_edu</span> 師: ${c.staff ? c.staff.name : '<span style="color:#9ca3af">未指派</span>'}</div>
          <div style="font-size:13px; color:var(--text-light);"><span class="material-symbols-outlined" style="font-size:14px; vertical-align:text-bottom;">assignment_ind</span> 導: ${c.tutor ? c.tutor.name : '<span style="color:#9ca3af">未指派</span>'}</div></td>
...
window.addScheduleSlot = () => {
  ...
  if (!start || !end) return window.showCustomDialog('提示', '請填寫完整的開始與結束時間', 'alert', 'info');
...
function renderScheduleBadges() {
  ...
  container.innerHTML = scheduleSlots.map((s, idx) => `<span class="schedule-badge">${s} <span class="material-symbols-outlined" onclick="window.removeScheduleSlot(${idx})" title="移除">cancel</span></span>`).join('')
}
...
window.openEditor = async (id = null) => {
  ...
  if (id) {
    document.getElementById('editor-title').textContent = '編輯班級與名單'; document.getElementById('roster-overlay').style.display = 'none'; document.getElementById('roster-content').style.display = 'block'
    ...
    if (c) {
      ...
      currentManageClassId = id; await loadRoster()
    }
...
classForm.addEventListener('submit', async (e) => {
  e.preventDefault(); const btn = document.getElementById('submit-btn'); btn.disabled = true; btn.innerHTML = '儲存中...'
  try {
    const id = document.getElementById('class-id').value
    if (scheduleSlots.length === 0) throw new Error('請至少建立一個上課時段！')
    ...
    if (!id && data) { currentManageClassId = data[0].id; document.getElementById('class-id').value = currentManageClassId; document.getElementById('roster-overlay').style.display = 'none'; document.getElementById('roster-content').style.display = 'block'; await loadRoster() }
    await window.showCustomDialog('成功', '班級基本資料儲存成功！\n您可以切換到「管理學生名單」頁籤繼續操作。', 'alert', 'check_circle'); await fetchClasses()
...
// 💡 雙向選單 (Dual Listbox) 邏輯
async function loadRoster() {
  const branchId = document.getElementById('branch_id').value;
  // 只載入同分校的學生，優化操作體驗
  let query = supabase.from('students').select('id, name, student_number').order('name');
  if (branchId) query = query.eq('branch_id', branchId);
  const { data: s } = await query; allStudents = s || [];
  
  const { data: r } = await supabase.from('class_students').select('student_id').eq('class_id', currentManageClassId)
  const selectedIds = r ? r.map(x => x.student_id) : []
  
  const avail = document.getElementById('avail-students'); const sel = document.getElementById('selected-students')
  avail.innerHTML = ''; sel.innerHTML = ''
  
  allStudents.forEach(st => {
    const opt = new Option(`${st.name} (${st.student_number || '無學號'})`, st.id)
    if(selectedIds.includes(st.id)) sel.appendChild(opt); else avail.appendChild(opt)
  })
}

window.filterAvail = () => { const k = document.getElementById('search-avail').value.toLowerCase(); Array.from(document.getElementById('avail-students').options).forEach(o => o.style.display = o.text.toLowerCase().includes(k)?'block':'none') }
window.moveToSelected = () => { const a = document.getElementById('avail-students'); const s = document.getElementById('selected-students'); Array.from(a.selectedOptions).forEach(o => { o.selected = false; s.appendChild(o); }) }
window.moveToAvail = () => { const a = document.getElementById('avail-students'); const s = document.getElementById('selected-students'); Array.from(s.selectedOptions).forEach(o => { o.selected = false; a.appendChild(o); }) }

window.saveClassRoster = async () => {
  try {
    const ids = Array.from(document.getElementById('selected-students').options).map(o => o.value)
    await supabase.from('class_students').delete().eq('class_id', currentManageClassId)
    if (ids.length > 0) await supabase.from('class_students').insert(ids.map(id => ({ class_id: currentManageClassId, student_id: id })))
    await window.showCustomDialog('成功', '班級學生名單儲存成功！', 'alert', 'check_circle')
