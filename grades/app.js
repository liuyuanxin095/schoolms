...
window.openExamEditor = async (examId = null) => {
  ...
  let existingScores = {}
  if (examId) { const { data: scores } = await supabase.from('grades').select('*').eq('exam_id', examId); if (scores) scores.forEach(s => existingScores[s.student_id] = s) }
  
  if (!students || students.length === 0) { rosterList.innerHTML = '<tr><td colspan="3" style="text-align:center;">班上尚無學生</td></tr>'; return }
  rosterData = students.map(s => s.students)
  rosterData.sort((a,b) => (a.student_number||'').localeCompare(b.student_number||'', 'zh-TW', {numeric:true}))
  
  rosterList.innerHTML = ''
  rosterData.forEach((s, idx) => {
    const sc = existingScores[s.student_id]; const scoreVal = sc && sc.score !== null ? sc.score : ''; const noteVal = sc && sc.note ? sc.note : ''
    rosterList.innerHTML += `
      <tr>
        ...
        <td><input type="text" class="row-note" value="${noteVal}" placeholder="選填"></td>
      </tr>`
  })
...
window.saveExamData = async () => {
  ...
  try {
    const examId = document.getElementById('exam-id').value; const eName = document.getElementById('exam_name').value; const subj = document.getElementById('subject').value; const eDate = document.getElementById('exam_date').value
    ...
    const examPayload = { class_id: currentClassId, exam_name: eName, subject: subj, exam_date: eDate, avg_score: avg==='-'?null:parseFloat(avg), high_score: high==='-'?null:parseFloat(high), low_score: low==='-'?null:parseFloat(low) }
    ...
    await window.showCustomDialog('成功', '成績儲存完畢！', 'alert', 'check_circle')
    hasUnsavedChanges = false; window.switchView('exams'); loadExamsList()
