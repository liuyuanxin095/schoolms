...
function renderTable(data) {
  ...
  data.forEach(s => {
    ...
    row.innerHTML = `
      <td><strong>${s.name}</strong> <span style="font-size:12px; color:var(--text-light);">${s.id_number || ''}</span></td>
      <td>${statusDisplay}</td>
      <td>${s.phone || '-'}</td>
      <td>${s.hire_date || '-'}</td>
      <td>${branchName}</td>
...
window.openFormModal = (id = null) => {
  ...
  if (id) {
    document.getElementById('form-title').textContent = '修改人事資料'
    ...
    if (s) {
      ...
      document.getElementById('emergency_contact').value = s.emergency_contact || ''; document.getElementById('emergency_phone').value = s.emergency_phone || ''
      document.getElementById('address').value = s.address || ''
    }
  } else { document.getElementById('form-title').textContent = '新增教職員人事檔案' }
...
staffForm.addEventListener('submit', async (e) => {
  ...
  try {
    ...
    window.closeFormModal(); await fetchStaff()
  } catch (err) { await window.showCustomDialog('錯誤', err.message, 'alert', 'error') } 
  finally { btn.disabled = false; btn.textContent = '儲存資料' }
})
